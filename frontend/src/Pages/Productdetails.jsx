...
const Productdetails = () => {
    ...
    useEffect(() => {
        fetch(`https://skin-care-hub.onrender.com/product/${id}`).then(res => res.json()).then(res => setPro(res)).catch(err => console.log(err))
    }, [id])
    console.log(pro)
    let rating = "";
    if (pro.rating === 5) {
    ...
    return (
        <SimpleGrid columns={[1, 1, 1, 1, 2]} className='product_details_container'>
            <Box>
                <Box>
                    <Image className='product_image' src={pro.image_link} alt={pro.name} width={["300px", '600px', '550px']} />
                </Box>
                <Box textAlign='left' >
                    <Text marginBottom='10px' fontSize={'18px'} fontWeight='600'>Description</Text>
                    <Text lineHeight='25px' fontSize={'14px'} fontFamily={'sans-serif'}>{pro.description}</Text>
                </Box>
            </Box>
            <Box textAlign='left' width='90%'>
                <Text fontSize='20px' fontWeight={'700'}>This {pro.name} is from {pro.brand} </Text>
                <Text display='flex' alignItems='center' fontSize={'14px'}><Image src={rating} alt={pro.rating} width='70px' height='56px' />(102 Rating | {pro.review} reviews)</Text>
                <Text fontSize={'20px'}><b>₹{pro.price * 75}</b> <span style={{ fontSize: '14px', color: 'green' }}>(save 112)</span></Text>
                ...
                <Box className='elite_member'>
                    <Image src='https://media6.ppl-media.com/mediafiles/ecomm/promo/1594961611_elite-logo.png' alt='elite' width={'35px'} height='50px' />
                    <Box>
                        <Text fontSize={'14px'}>Elite members get free shipping on this purchase.</Text>
                        <Text color={'blue.400'} fontWeight='bold'>Join Now</Text>
                    </Box>
                </Box>
                <Box className='pincodediv'>
                    <Box display={'flex'}>
                        <Image width='30px' height='30px' src='https://cdn-icons-png.flaticon.com/128/9335/9335511.png' alt='truck' />
